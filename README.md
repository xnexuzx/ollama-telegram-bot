<div align="center">
  ...
  <h1>🦙Ollama Telegram Bot [YAIS-NXZ] v1.0.5</h1>
  <p>
    <b>Chatea con tus Modelos de Lenguaje (LLMs) directamente desde Telegram.</b><br>
    Una interfaz de bot potente, eficiente y altamente configurable para Ollama con persistencia completa y mejoras UX.
  </p>
  <br>
...
- **Persistencia Completa**: Tus selecciones (modelos, prompts, configuraciones) se guardan automáticamente y se restauran al reiniciar.
...
- **División Inteligente de Mensajes**: Divide automáticamente las respuestas largas manteniendo la coherencia.
...
- **Spinner Animado Robusto**: Indicador de progreso con limpieza automática en errores y estados por usuario.
...
- **Logging Estructurado**: Sistema de registro profesional para monitoreo y depuración.

### 🔧 Administración y Seguridad

- **Gestión de Modelos Completa**: Administra modelos de Ollama (`pull`, `rm`) directamente desde la interfaz del bot.
...
- **Personalización Persistente**:
    ...
    - **Selección de Modelo**: Elige y guarda tu modelo de Ollama preferido.
...
- `/prompts`: Permite seleccionar un "system prompt" para cambiar la personalidad del bot. **La selección es persistente** y se guarda automáticamente.
...
- **Selección de Modelo**: Tu modelo de Ollama preferido se guarda automáticamente y se restaura al reiniciar el bot.
...
- **Gestión de Modelos**:
    - **Cambiar el LLM activo**: Seleccionar qué modelo de Ollama usar (se guarda como predeterminado para todos los usuarios).
...
## 🏗️ Mejoras Técnicas (v1.0.5)

La versión 1.0.5 incluye una refactorización completa que mejora significativamente la calidad y mantenibilidad del código:

### Estructura y Organización

- **Paquete `bot/utils/` reestructurado**: Módulos separados para diferentes funcionalidades.
- **Clase `SpinnerManager`**: Gestor centralizado de indicadores de progreso con limpieza automática.
...
- **Type hints completos**: Tipado fuerte en todo el código base.
...
- **Validación de entradas**: Comprobaciones adicionales para mayor seguridad.
- **Gestión de memoria**: Limpieza automática de recursos para prevenir fugas.
